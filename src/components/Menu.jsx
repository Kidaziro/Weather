...
const Menu = (props) => {
    ...
    return (
      <div className="z-[1] ">
        ...
        />
        <div
          className={`p-10  ${isVisible ? "visible" : "hidden"} absolute flex flex-col items-start`}
          id="search"
        >
          ...
          <div className="backdrop-blur-md  text-white border-b-[1px] border-l-[1px] border-r-[1px] rounded-b-2xl lg:min-w-[350px] md:max-w-[250px] sm:max-w-[200px] max-w-[245px] ">
            {data.map((locationData, index) => (
              ...
              >
                ...
                <div className="flex justify-between cursor-pointer" id={`[ ${locationData.lat} , ${locationData.lon} ]`} onClick={handleClick} >
                  <h2 className="font-bold lg:text-base md:text-sm sm:text-xs text-lg">{locationData.name}</h2>
                  
                    <p className=' lg:text-base md:text-sm sm:text-xs text-xs ml-1'>
                      Country: {locationData.country} 
                      <Flag code={locationData.country} className="lg:w-[30px] md:w-[20px] sm:w-[20px] w-[35px] ml-auto" />
                    </p>
                ...
                <div className="flex mt-2 lg:mb-2 md:mb-3 sm:mb-2 mb-2">
                  <p className="lg:text-xs md:text-[9px] sm:text-[8px] text-[8px] ">Latitude: {locationData.lat}</p>
                  <p className="lg:text-xs md:text-[9px] sm:text-[8px] text-[8px] lg:ml-3 md:ml-10 sm:ml-10 ml-11">Longitude: {locationData.lon}</p>
                </div>
              </div>
            ))}
          </div>
